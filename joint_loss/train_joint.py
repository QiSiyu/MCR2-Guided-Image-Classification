...
'''Train a modified ResNet with 2-phase hierarchical strategy:
    phase 1: MCR2 loss on lower level modules;
    phase 2: CE loss on higher level modules'''
...
print('Weight of cross entropy loss: %.1f' % (args.ce_lam))
...
for epoch in range(pure_ce_epoch):
    ...
    for step, (batch_imgs, batch_lbls) in enumerate(ce_trainloader):
        ...
        # loss, loss_empi, loss_theo = criterion(features, batch_lbls, num_classes=num_classes)
        loss.backward()
