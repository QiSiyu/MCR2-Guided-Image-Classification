...
def load_architectures(name, dim, im_shape = 32,dropout=False):
    ...
    if _name == "resnet18dwt":
        from architectures_joint.resnet_cifar_joint import ResNet18DWT
...
def load_checkpoint(model_dir, epoch=None, eval_=False, im_shape=32,dropout=False):
    ...
    if 'mcr2_optimizer' in checkpoint:
        ...
        return net, checkpoint['optimizer'], None, checkpoint['scheduler'], None, epoch

def load_part_of_model(model_dir, epoch=None, im_shape=32):
    if model_dir.endswith('pt'):
        ckpt_path = model_dir
        epoch=0
    elif epoch is None: # get last epoch
...
def load_prune_model(model_dir, epoch=None, im_shape=32):
    if model_dir.endswith('pt'):
        ckpt_path = model_dir
        epoch=0
    elif epoch is None: # get last epoch
