...
def load_architectures(name, dim, im_shape = 32, mult=1):
    ...
    if _name == "resnet18":
        ...
        net = ResNet18(dim, im_shape=im_shape)
    elif _name == "resnet18dct":
        from architectures.resnet_cifar import ResNet18DCT
        net = ResNet18DCT(dim, im_shape=im_shape)
    elif _name == "resnet18dwt":
        from architectures.resnet_cifar import ResNet18DWT
        net = ResNet18DWT(dim, im_shape=im_shape, mult=mult)
    elif _name == "resnet18dwtr":
        from architectures.resnet_cifar import ResNet18DWT_eval
        net = ResNet18DWT_eval(dim, im_shape=im_shape, mult=mult)
...
def load_checkpoint(model_dir, epoch=None, eval_=False, im_shape=32):
    ...
    if params['DCT'] and (not arc.endswith('DCT')):
        ...
        arc += 'dwt'
    if ('eval' in params.keys()) and params['eval']:
        arc += 'r'
    net = load_architectures(arc, params['fd'],im_shape)
...
def load_part_of_model(model_dir, epoch=None, eval_=False, im_shape=32, build_eval_model=True):
    if epoch is None: # get last epoch
        ckpt_dir = os.path.join(model_dir, 'checkpoints')
        epochs = [int(e[11:-3]) for e in os.listdir(ckpt_dir) if e[-3:] == ".pt"]
        epoch = np.sort(epochs)[-1]
